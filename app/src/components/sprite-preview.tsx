...
export default function SpritePreview(props:{sprite: Sprite, dungeon: Dungeon, animDataUrl: string}) {
    ...
    const container = useCallback((node: HTMLDivElement) =>{
        async function initialize(){
            ...
            new GameContainer(node as HTMLDivElement, props.sprite, data.AnimData, props.dungeon)
        }
    ...
    return <div className='my-container nes-container' style={{display:'flex', alignItems:'center', flexFlow:'column', margin:'10px'}}>
            ...
            <p style={{fontSize: '0.6em', margin: '0px'}}>{props.sprite.action}</p>
        </div>
}
