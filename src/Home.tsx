...
export type ShowParameters = Record<
  string,
  {
...
export default function Home({ ids, meta }: { ids: number[]; meta: Meta }) {
  ...
  const showParameters: ShowParameters = {
    index: {
      ...
      rankMethod: RankMethod.POKEDEX_NUMBER,
      name: "Index"
    },
    portraitAuthor: {
      state: useState<boolean>(false),
      rankMethod: RankMethod.PORTRAIT_AUTHOR,
      name: "Portrait Author"
    },
    spriteAuthor: {
      state: useState<boolean>(false),
      rankMethod: RankMethod.SPRITE_AUTHOR,
      name: "Sprite Author"
    },
    lastModification: {
      state: useState<boolean>(false),
      rankMethod: RankMethod.LAST_MODIFICATION,
      name: "Last Change"
    },
    portraitBounty: {
      state: useState<boolean>(false),
      rankMethod: RankMethod.PORTRAIT_BOUNTY,
      name: "Portrait Bounty"
    },
    spriteBounty: {
      state: useState<boolean>(false),
      rankMethod: RankMethod.SPRITE_BOUNTY,
      name: "Sprite Bounty"
    },
    fullyFeaturedSprites: {
      state: useState<boolean>(false),
      name: "Fully-Featured Sprites"
    },
    fullyFeaturedPortraits: {
      state: useState<boolean>(false),
      name: "Fully-Featured Portraits"
    }
  }
    ...
    <Box>
      ...
      <Container maxWidth="xl" sx={{ backgroundColor: "rgba(255,255,255,.9)" }}>
        <Typography
          ...
          gutterBottom
        >
          Free to use <strong>WITH CREDIT</strong> for ROMhacks, fangames, etc.
          Don't use for commercial projects.
        </Typography>
